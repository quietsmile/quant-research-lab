...
def is_a_share(symbol: str) -> bool:
    ...
    沪(6 开头:600/601/603/605/688)、深(0/3 开头:000/002/003/300/301)与新三板
    (4/8 开头)、B 股(2/9 开头)代码空间不重叠，故 0/3/6 开头可无歧义判定为沪深 A 股，
    且不依赖"当前是否在市"，**不引入幸存者偏差**（已退市的沪深 A 股代码仍被识别）。

    注意：**北交所**(8/4/920 开头)与新三板共用前缀，无法靠代码区分，故本函数
    不含北交所；如需北交所，请用交易所权威清单单独标记。
    """
    return str(symbol).zfill(6)[0] in ("0", "3", "6")
